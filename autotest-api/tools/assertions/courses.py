from clients.courses.courses_schema import UpdateCourseResponseSchema, UpdateCourseRequestSchema, GetCoursesResponseSchema, CreateCourseResponseSchema, CourseSchema, CreateCourseRequestSchema
...
def assert_get_courses_response(
    ...
    for index, create_course_responses in enumerate(create_course_responses):
        asser_course(get_courses_response.courses[index], create_course_responses.course)
...
def assert_create_course_response(request: CreateCourseRequestSchema, response: CreateCourseResponseSchema):
    ...
    """
    Проверяет, что запрос на создание курса соответствует ответу создания курса.
    :param request: Запрос на создание курса.
    :param response: Ответ на создание курса.
    """
    assert_equal(response.course.title, request.title, "title")
    assert_equal(response.course.max_score, request.max_score,"max_score")
    assert_equal(response.course.min_score, request.min_score, "min_score")
    ...
    assert_equal(response.course.preview_file.id, request.preview_file_id,"preview file id")
    assert_equal(response.course.created_by_user.id, request.created_by_user_id, "user id")
